...
def validate_birthday(date_str=None):
    """Проверяет корректность даты в формате ГГГГ-ММ-ДД."""
    while True:
        # Если дата передана (из файла), проверяем её. 
        # Если нет (ручной ввод), запрашиваем у пользователя.
        birthday = date_str if date_str else input("Birthday (YYYY-MM-DD): ").strip()
        try:
            datetime.strptime(birthday, '%Y-%m-%d')
            return birthday
        except ValueError:
            if date_str: # Если ошибка пришла из CSV файла, возвращаем None
                return None
            print("Ошибка: Неверный формат даты! Используйте ГГГГ-ММ-ДД (например, 2000-01-01).")
            date_str = None



def import_csv():
    filename = input("CSV filename (например, contacts.csv): ").strip()
    try:
        with open(filename, "r", encoding="utf-8") as file:
            ...
            with connect() as conn:
                with conn.cursor() as cur:
                    for row in reader:
                        # Проверяем дату из файла перед вставкой
                        bday = validate_birthday(row["birthday"])
                        if not bday:
                            print(f"Пропуск строки {row['name']}: некорректная дата {row['birthday']}")
                            continue
                        ...
                        cur.execute("""
                            ...
                            ON CONFLICT (name) DO UPDATE SET
                                surname = EXCLUDED.surname, email = EXCLUDED.email,
                                birthday = EXCLUDED.birthday, group_id = EXCLUDED.group_id
                            RETURNING id
                        """, (row["name"], row["surname"], row["email"], bday, group_id))
                ...
                conn.commit()
        print("Импорт из CSV завершен.")
    except Exception as e:
        print(f"Ошибка импорта: {e}")

def export_csv():
    """Добавляем эту функцию, чтобы сохранять изменения из базы обратно в CSV"""
    filename = input("Имя файла для экспорта (например, contacts_updated.csv): ").strip()
    with connect() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT c.name, c.surname, c.email, c.birthday, g.name, 
                       string_agg(p.phone, ';'), string_agg(p.type, ';')
                FROM contacts c
                LEFT JOIN groups g ON c.group_id = g.id
    ...
    with open(filename, "w", newline="", encoding="utf-8") as file:
        ...
        writer.writerow(["name", "surname", "email", "birthday", "group", "phone", "type"])
        writer.writerows(rows)
    print(f"Все данные из базы сохранены в файл {filename}")
...
def import_json():
    ...
    with connect() as conn: # with psycopg2.connect(**config) as conn:
        with conn.cursor() as cur:
            for item in data:
                name = item["name"]
...
def show_all_contacts():
    """Показывает все контакты с их телефонами и группами."""
    with connect() as conn:
        with conn.cursor() as cur:
            cur.execute("""
